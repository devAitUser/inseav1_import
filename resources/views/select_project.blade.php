...
<div style="background-color: #2b7239" class="panel-heading"><strong>  Selectionner la Division </strong> </div>
              ...
              <div class="col-md-12">
                     ...
                     <!-- delet code-->
                                ...
                                <?php
                                       ...
                                       // tableau pour stocker les IDs des projets déjà affichés
                                       for ($i = 0; $i < count($projets); $i++)
                                     {
                                          // Vérifie si l'ID du projet a déjà été affiché avant d'afficher le bouton de projet
                                          if (!in_array($projets[$i]['id'], $already_displayed)) 
                                          {
                                             array_push($already_displayed, $projets[$i]['id']); 
                                             // ajoute l'ID du projet au tableau des projets déjà affichés
                                             $class = ($projets[$i]['id'] == $user['projet_select_id']) ? 'active' : ''; 
                                             // classe "active" si c'est le projet sélectionné
                                             echo '<button type="button" id_division="' . $projets[$i]['id'] . '" class="btn btn-danger mt-1 ' . $class . ' btn_project">' . $projets[$i]['nom'] . '</button>';
                                          }
                                    }
                                ?>
